...
class DescriptionLanguage implements IILanguageBlock {
    ...
    public static final ILanguageString DATABASE_IMPL = ILanguageString.Companion.create("数据库实现，插件默认使用 sqlite 作为数据库。当该值为 default-sqlite 时使用默认 SQLite 实现，值为 default-mysql 时使用默认 MySQL 实现，若有其他数据库实现，需填写其 jar 包的绝对路径");
...
//            "当值为 EDITING (编辑中) 时，表明该区域下有游戏正在被编辑，自动保存将会对该区域启用，玩家可以仅可以进入状态为 ENABLED (已启用) 的游戏，" +
    ...
    public static final ILanguageString AREA_COORDINATE_CHANGE = ILanguageString.Companion.create("区域坐标属性修改，可接受四个参数，参数一为需要修改的属性，后面的参数二，三，四分别为坐标的 x, y, z。可修改的属性为 lobby-spawnpoint-coordinate");
